...
predict_leaf.DTR <- function(treeout, newdata){
  # n<-nrow(newdata)
  # predicts<-rep(NA,n)
  n <- nrow(newdata)
  ...
  if (nrow(treeout) == 1) {
    ...
    for (i in 1:n) {
      ...
      while ( length(treeout$X[treeout$node == nd])>0 ) {
        
        
        # if this node have NO opt.trt assigned  
        # Modification : if split is numerical
        if(is.na(as.numeric(treeout$X[nd]))){
          break
        }
